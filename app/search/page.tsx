    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-12">
        <div className="space-y-4 text-left">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-amber-500/10 rounded-full text-amber-600 text-xs font-black uppercase tracking-wider">
          ...
          <p className="text-slate-500 text-lg font-medium max-w-xl leading-relaxed">
            Finding your partner within <span className="text-slate-900 font-bold underline decoration-amber-500/30 decoration-4 underline-offset-4">{community}</span>.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="flex -space-x-3">
            ...
            <div className="w-10 h-10 rounded-full border-2 border-white bg-slate-900 flex items-center justify-center text-white text-[10px] font-black">
              {profiles.length}+
            </div>
          </div>
      ...
      <div className="bg-white rounded-[2.5rem] shadow-2xl shadow-slate-200/50 border border-slate-100 p-3 mb-16 flex flex-col lg:flex-row gap-3">
        <div className="flex-1 relative group">
          ...
          <input
            ...
            className="w-full pl-16 pr-6 py-4.5 bg-slate-50 border-none rounded-[2rem] focus:ring-2 focus:ring-amber-500/10 outline-none transition-all font-medium text-slate-700 placeholder:text-slate-400"
          />
        </div>

        <div className="lg:w-64 relative group">
          <MapPin className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-slate-900 transition-colors" size={20} />
          <select
            value={cityFilter}
            onChange={(e) => setCityFilter(e.target.value)}
            className="w-full pl-16 pr-10 py-4.5 bg-slate-50 border-none rounded-[2rem] appearance-none focus:ring-2 focus:ring-slate-900/10 outline-none font-bold text-slate-700 cursor-pointer"
          >
        ...
        <div className="bg-white rounded-[4rem] border border-slate-100 shadow-sm p-24 text-center space-y-8">
          ...
          <div className="space-y-3">
            ...
            <p className="text-slate-500 max-w-md mx-auto font-medium text-lg leading-relaxed">
              Try broadening your search criteria to find more potential TruSathi matches.
            </p>
          </div>
          <button
...
function SearchPageContent() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50/50">
      ...
      <div className="bg-slate-950 pt-20 pb-32 px-4 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-[800px] h-[800px] bg-amber-500/5 rounded-full -translate-y-1/2 translate-x-1/2 blur-[150px]" />
        <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-slate-700/10 rounded-full translate-y-1/2 -translate-x-1/2 blur-[100px]" />

        <div className="max-w-7xl mx-auto relative z-10 text-center">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-white/5 border border-white/10 rounded-full text-slate-300 text-xs font-black uppercase tracking-[0.2em] mb-8 backdrop-blur-md">
      ...
      <div className="grow -mt-20 relative z-20">
        <Suspense fallback={<div className="p-32 text-center"><Loader2 className="animate-spin mx-auto text-amber-500 w-12 h-12" /></div>}>
          <SearchResults />
        </Suspense>
      </div>
